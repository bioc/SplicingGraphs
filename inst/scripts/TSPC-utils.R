...
### Returns a GappedAlignments or GappedAlignmentPairs object, or NULL if the file
...
load_TSPC_bam <- function(subdir_path, sample_name, gapped.reads.only=FALSE)
{
    ...
    if (bam_status == "p") {
        reads <- readGappedAlignmentPairs(bam_filepath, use.names=TRUE,
                                     param=param0)
    ...
    reads <- readGappedAlignments(bam_filepath, use.names=TRUE, param=param0)
...
### Returns a GappedAlignments or GappedAlignmentPairs object.
