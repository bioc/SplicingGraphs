...
\Rfunction{readGappedAlignmentPairs} function from the \Rpackage{GenomicRanges}
...
galp <- readGappedAlignmentPairs(bam_files[1], param=param)
length(galp)  # nb of alignment pairs
galp
@

The result is a \Rclass{GappedAlignmentPairs} object with 1 element per
alignment pair (alignment pair = pair of BAM records). Although paired-end
reads could also be loaded with \Rfunction{readGappedAlignments} (also from the
\Rpackage{GenomicRanges} package), they should be loaded with the
\Rfunction{readGappedAlignmentPairs}: this actually pairs the BAM records
and returns the pairs in a \Rclass{GappedAlignmentPairs} object. Otherwise the
records won't be paired and will be returned in a \Rclass{GappedAlignments} object.
...
that we'll pass to \Rfunction{readGappedAlignmentPairs} (see
...
for (i in seq_along(bam_files)) {
    ...
    galp <- readGappedAlignmentPairs(bam_file, use.names=TRUE, param=param0)
...
reads <- readGappedAlignmentPairs(bam_files[1], use.names=TRUE, param=param)
